      ...
      {majorIds.map((majorId) => {
        ...
        return (
          ...
          >
            ...
            >
              <ProgressRing percentage={pctWithIP} />
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-0.5 text-xs text-gray-400">
                  <span>
                    ...
                    /{prog.totalCredits} credits completed
                  </span>
                  <span className="text-purple-300">
                    {pctWithIP.toFixed(0)}% with planned
                  </span>
                </div>
              </div>
            ...
            <div className="px-4 pb-3 -mt-1">
              <div className="w-full bg-gray-800/60 h-2 rounded-full overflow-hidden relative">
                <div
                  className="h-2 bg-gradient-to-r from-purple-400 to-purple-500 opacity-40 absolute top-0 left-0 rounded-full"
                  style={{ width: `${Math.min(100, pctWithIP)}%` }}
                  aria-hidden
                />
                <div
                  className="h-2 bg-gradient-to-r from-blue-400 to-blue-500 relative rounded-full"
  ...
  return (
    <div>
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        <span
          className={`text-xs font-medium uppercase tracking-wide ${titleColor[color]}`}
        >
          {title}
        </span>
        <span className="text-xs text-gray-600">({reqs.length})</span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
        {reqs.map((req) => {
          ...
          return (
            <div
              key={req.name}
              className="px-3 py-2 rounded-lg bg-gray-800/40 border border-gray-800/60"
            >
