...
1. **Courses are owner-only read** - Grades are NEVER exposed to anyone else
2. **Friends public data** - Only contains course codes, no grades. Only friends can read.
3. **Friends lookup** - Enables efficient friendship checks in security rules
4. **All writes require ownership** - Users can only modify their own data
